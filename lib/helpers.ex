defmodule Noizu.Core.Helpers do
  ...
  def ok?({:ok, v}, _), do: v
  def ok?(result = {:error, _}, requirement), do: raise Noizu.Core.ResultException, result: result, requirement: requirement
  def ok?(result, requirement), do: raise Noizu.Core.ResultException, result: {:error, {:invalid_tuple, result}}, requirement: requirement
